...
%% [DRAFT - VERIFY]
Final PRISMA counts and quantitative synthesis statements are still pending verification against the frozen search exports, deduplication output, and screening log. The current manuscript therefore retains draft narrative structure while deferring corpus-size claims and evidence totals until the review artifacts are committed.
\end{abstract}
...
\textbf{[DRAFT] PRISMA 2020 flow summary placeholder.} Insert the final identification, deduplication, screening, full-text exclusion, and inclusion counts after freezing the raw search exports, deduplication report, and screening log.
}}
...
%% [DRAFT - VERIFY]
Figure~\ref{fig:prisma-flow} is intentionally left as a structured placeholder in this repository snapshot. The raw search exports, deduplicated master corpus, and screening log needed to report verified PRISMA counts are not yet present in the committed artifacts, so this subsection should be finalized directly from those files once they are frozen.

%% [DRAFT - VERIFY]
At finalization time, this section should report records identified per source, duplicates removed, title/abstract exclusions, full-text exclusions by reason, and the final number of included studies. Until the screening artifacts are available in-repository, any exact numeric claims would be non-reproducible and are therefore withheld from the canonical manuscript.
