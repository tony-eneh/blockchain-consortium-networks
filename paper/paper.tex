...
\IEEEPARstart{D}{istributed} ledger technology has evolved well beyond its origins in public cryptocurrency networks~\cite{nakamoto2008bitcoin,wood2014ethereum}.
...
Shared immutable records can reduce reconciliation costs in inter-bank settlement~\cite{guo2016blockchain}, trade finance and supply-chain provenance~\cite{chang2020supplychain,malik2019trustchain,montecchi2021survey,jadon2024electronics}, and cross-border payment corridors~\cite{danezis2015centrally}.
...
In the public sector, multi-agency ledgers have been explored for land registries, digital identity, vaccination coordination, and health-data exchange where no single entity is trusted to maintain the canonical record~\cite{olnes2017blockchain,mcghin2019blockchain,costa2022vaccination,tahir2024healthcare,villarreal2023healthcareinterop}.
...
Crash-fault-tolerant protocols such as Raft offer high throughput but assume benign failures, while Byzantine-fault-tolerant protocols such as PBFT, Istanbul~BFT, HotStuff, and newer randomized or sharded variants provide stronger resilience guarantees at the cost of communication complexity and latency~\cite{ongaro2014raft,castro1999practical,sousa2018byzantine,baudet2019state,miller2016honeybadger,yin2019hotstuff}.
Choosing the right consensus mechanism for a given consortium size, failure model, and throughput target remains an open engineering problem, because benchmarks are conducted under heterogeneous workloads, network topologies, and hardware configurations that resist direct comparison~\cite{dinh2017blockbench,nasir2022performance,wu2023sharding,zhong2023stpbft}.

Second, \emph{governance models} define who can join the network, how policies are updated, and how disputes are resolved.
These models range from static configuration files managed by a founding operator, through multi-signature policy channels, to on-chain decentralized-autonomous-organization (DAO) patterns adapted for permissioned contexts~\cite{beck2018governance,reijers2021governance,zhang2023governance,badi2025governance}.
...
Patterns include cross-chain atomic swaps, relay chains, hash-time-locked contracts (HTLCs), API-mediated gateways, oracle networks, and event-driven messaging buses~\cite{belchior2021survey,schulte2019towards,ren2023interoperability,madine2021appxchain,alhussayen2024oracle,patsonakis2019crosschain}.
...
\subsection{Study Characteristics and Quality Distribution}

The 279 included studies exhibited substantial heterogeneity in study quality and rigor. Across the GRADE-informed four-dimension assessment (construct validity, internal validity, external validity, and reproducibility), quality scores ranged from 0 to 8. The final tier distribution was: \textbf{Tier A (score 6--8): 26 studies} (9\% of corpus), \textbf{Tier B (4--5): 122 studies} (44\%), \textbf{Tier C (2--3): 110 studies} (39\%), and \textbf{Tier D (0--1): 21 studies} (8\%). This distribution—with fewer than 10\% achieving high reproducibility marks—underscores the persistent gap between aspirational blockchain papers and those with artifact availability, clear evaluation protocols, and deterministic setups. An extraction sheet has already been generated in the repository for the 863 candidate studies that survived first-pass screening; the 279-record post-full-text agent inclusion set is a strict subset of that sheet and provides the working population for data extraction and detailed quality re-scoring in the next review iteration.

To make the synthesis traceable to the included-study base, the expanded reference set now cites representative high-scoring studies across healthcare records, IoMT, medical certificates, privacy-preserving health-data exchange, and patient-controlled cloud records~\cite{sasikumar2023heart,sammeta2021medical,thakur2026bioblock,khan2022biomt,tellew2022certificatechain,mohammadi2024scalhealth,davids2024gpoc,babu2021ehr,pandey2023healthiot,yao2024crossdomain,khan2025iomt,saxena2025chronic,alsalamah2025embryotrust,myeong2025zkphealth,ali2022homomorphic,li2022proxy}. It also anchors public-sector, audit, education, incident-reporting, and institutional accountability examples~\cite{intud2025incident,zhang2025auditing,dash2025certificate,alqodsi2025procurement,anichukwueze2021recordkeeping,wang2025exam,norainunnajah2025fertilizer}, specialized governance and privacy cases such as voting, sealed auctions, and collaborative drug discovery~\cite{schiedermeier2024anonymous,zhan2025auction,olsson2021drugdiscovery}, supply-chain, transport, finance, and logistics studies~\cite{jabbar2022its,allam2025crossborder,ballot2021physicalinternet,karkanias2026agrifood,yiu2021anticounterfeiting}, and system-level work on consensus, smart-contract architectures, cooperative IoT, and hospital information systems~\cite{fu2026flyingblocks,lin2022smartcontractsurvey,alfuhaid2023mapping,erukala2025cooperativeiot,leng2021hyperledgerhospital,iqbal2021veterinary}.
...
Across the 279 included studies, the most common domains were healthcare (143), supply chain (124), government/public sector (123), and finance/banking (103), with representative studies ranging from health-information interoperability and vaccination workflows to electronics supply-chain traceability~\cite{manoharan2024healthinterop,costa2022vaccination,tahir2024healthcare,jadon2024electronics}.
...
Network model distribution across the corpus showed that consortium architectures dominated institutional deployments:
...
\begin{tabular}{lr}
\toprule
\textbf{Network Model} & \textbf{Studies} \\
\midrule
Consortium & 79 \\
Permissioned & 57 \\
Public & 44 \\
...
\bottomrule
\end{tabular}
\caption{Network model distribution in the 279 included studies.}
\label{tab:network-models}
\end{table}

\textbf{Privacy Mechanisms.}
Privacy strategy adoption showed that institutional deployments favored compositional approaches combining multiple layers:
...
\begin{tabular}{lr}
\toprule
\textbf{Privacy Mechanism} & \textbf{Studies} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Privacy mechanism frequency in the corpus.}
\label{tab:privacy-mechanisms}
\end{table}

\noindent Key observation: Zero-knowledge proofs and homomorphic encryption were more frequently discussed in theory than deployed in practice, while private channels and encryption-based approaches dominated operational consortium deployments.
...
\textbf{Domain-Platform Pairing Analysis.}
Cross-tabulation of application domain and platform choice revealed platform specialization patterns driven by native capabilities: Hyperledger Fabric dominated healthcare (24 studies reporting Fabric in healthcare context) and supply-chain (18 studies), where channel-based privacy and modular endorsement proved attractive~\cite{manoharan2024healthinterop,tahir2024healthcare,villarreal2023healthcareinterop}. R3~Corda concentrated in finance/banking (9 studies) where selective data minimization and legal-process alignment were critical. Ethereum variants (Quorum, Besu) appeared more frequently in government/public-sector and energy-utility deployments (12 studies combined) where Ethereum compatibility and faster operational onboarding outweighed consortium-specific privacy gains. Bitcoin appeared primarily in conceptual/policy studies rather than operational consortium pilots (55 studies reporting Bitcoin were predominantly categorized as C-D tier in quality).
...
The included evaluations consistently reported that throughput, resilience, and privacy cannot be simultaneously maximized without increasing design complexity. Studies based on Hyperledger Fabric frequently showed that crash-fault-tolerant ordering with limited endorsement sets provides the strongest raw throughput, particularly when block sizes and batching intervals are tuned for steady workloads~\cite{dinh2017blockbench,gorenflo2020fastfabric,thakkar2018performance,nasir2022performance,dadkhah2025tuning,nunes2025permissioned}. However, these gains weaken when endorsement policies expand across more organizations or when channel fragmentation increases coordination overhead. Conversely, BFT-style ordering improves resilience to malicious or arbitrary faults, but message complexity and quorum coordination typically produce higher latency and lower sustained throughput as consortium size grows~\cite{sousa2018byzantine,castro1999practical,crain2018dbft,wu2023sharding,zhong2023stpbft}.

\textbf{Metric Coverage and Ranges.}
Metric coverage remains sparse at corpus scale, but the studies that do report provide concrete evidence of trade-offs:
...
\begin{tabular}{lrr}
\toprule
\textbf{Metric} & \textbf{Studies} & \textbf{\% of Corpus} \\
\midrule
Throughput (TPS) & 23 & 8\% \\
Latency & 33 & 11\% \\
Fault tolerance & 48 & 17\% \\
Evaluation setup & 80 & 28\% \\
\bottomrule
\end{tabular}
\caption{Metric reporting frequency across 279 included studies.}
\label{tab:metric-coverage}
\end{table}

\noindent For reported throughput, observed ranges varied widely depending on consensus and configuration: Fabric CFT deployments achieved 50--300 TPS under Caliper benchmarks with standard block intervals, while BFT-ordered configurations showed 25--100 TPS. Outlier high-throughput studies (500--1700 TPS) typically employed custom workloads, optimized parameters (e.g., tuned batching), or smaller consortium sizes (2--4 organizations). Low-throughput results (3--10 TPS) often reflected privacy-heavy designs or multi-step workflows with external system calls. Latency measurements similarly spanned three orders of magnitude: sub-millisecond latencies (1--30 ms) under synthetic loads, typical responses of 100--600 ms for real institutional workflows, and failure-recovery or multi-step governance operations extending to seconds or longer (up to 250+ seconds in some interoperability scenarios).
...
\begin{tabular}{lr}
\toprule
\textbf{Interoperability Pattern} & \textbf{Studies} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Distribution of interoperability patterns (48 studies total with explicit mechanisms).}
\label{tab:interop-patterns}
\end{table}

\noindent The pattern distribution reflects institutional pragmatism: oracle and API-gateway approaches (together 17/48 studies, 35\% of those reporting interoperability) dominated because they fit within existing enterprise governance, audit, and exception-management practices. Cross-chain bridges, while frequently proposed in conceptual architectures (25 studies), were overwhelmingly described rather than evaluated in mature institutional pilots. Relay and HTLC patterns were limited to specific use cases (inter-bank settlement, multi-consortium asset transfer) where atomic semantics were critical.

Relay and HTLC approaches were more frequently proposed in conceptual interoperability architectures than in mature institutional pilots. Their main advantage was stronger transactional coupling across heterogeneous ledgers, which is attractive in inter-bank settlement or cross-consortium asset transfer. However, the reviewed studies repeatedly highlighted limitations related to timeout management, failure recovery, governance alignment across chains, and the difficulty of mapping business-finality requirements onto protocol-level atomicity semantics~\cite{herlihy2018atomiccrosschain,patsonakis2019crosschain}. In settings such as insurance claims exchange or regulated reporting, eventual consistency through auditable gateways was often preferred over strict cross-chain atomic commit.
...
\begin{tabular}{lrr}
\toprule
\textbf{Artifact Type} & \textbf{Studies} & \textbf{\% of Corpus} \\
\midrule
Code (GitHub / open-source) & 35 & 12\% \\
Configuration / deployment & 18 & 6\% \\
Dataset & 6 & 2\% \\
\bottomrule
\end{tabular}
\caption{Artifact availability in the included studies.}
\label{tab:artifacts}
\end{table}

\noindent This reproducibility deficit, together with sparse metric reporting (8\% throughput, 11\% latency), is the strongest cross-cutting limitation in the corpus.
...
\textbf{Identified Gaps from Full-Text Extraction.}
Five systematic limitations emerged from the full-text analysis of the 95 papers (34\% of corpus) where open-access extraction was performed:

\begin{enumerate}
  \item \textbf{Benchmarking standardization:} Most studies used custom or ad-hoc workloads; only a minority employed Hyperledger Caliper or YCSB~\cite{hyperledger2024caliper}. Cross-study performance comparison was therefore impossible.
  \item \textbf{Governance formalization:} Governance models were rarely specified explicitly; consortium rules were typically implicit or described informally in text.
  \item \textbf{Interoperability specification:} Cross-chain and bridge mechanisms were underspecified; most studies assumed single-platform deployments.
  \item \textbf{Privacy-performance trade-off quantification:} Studies mentioning privacy mechanisms rarely quantified computational overhead, attestation costs, or key-management burden.
  \item \textbf{Deployment reproducibility:} Docker/Kubernetes configuration files, terraform scripts, and deployment automation were almost never published alongside papers.
\end{enumerate}
